...
const DeleteTab = () => {
  ...
  // Get the first string field to display (fallback to 'id')
  const stringFields =
    table_fields
      ?.filter(field => field.type === 'string' && field.name)
      .map(field => field.name) || [];

  const displayField = stringFields.length > 0 ? stringFields[0] : 'id';
        ...
        <form onSubmit={onSubmit}>
          <DialogBody>
            ...
            <h2 className="text-center text-2xl font-semibold">
              {selected${modelName}?.${displayField}}
            </h2>
          </DialogBody>
...
export default Delete${modelName}Dialog;`;
  ...
  const code = generateDeleteDialog();
