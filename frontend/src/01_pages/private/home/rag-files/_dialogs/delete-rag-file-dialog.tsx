...
const DeleteRagFileDialog = ({
  open,
  setOpen,
  refetch,
}: DeleteRagFileDialogProps) => {
  // Zustand store
  ...
  const onSubmit = (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    toast.promise(mainInstance.delete(`/rag/files/${selectedRagFile?.id}`), {
      loading: 'Loading...',
      success: () => {
        refetch();
        setOpen(false);
      ...
      <DialogContent>
        <form onSubmit={onSubmit}>
          <DialogBody>
            ...
            {/* Modal title */}
            <h3 className="text-center text-xl">Delete Rag File</h3>
...
export default DeleteRagFileDialog;
